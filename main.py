...
def ensure_consistency_of_items_across_datasets():
    def get_interaction_matrix(item_list, user_list, interations):
        ...
        indices = torch.tensor(
            [index_row, index_col], dtype=torch.long
        )  # 行索引和列索引
        interaction_matrix = torch.sparse_coo_tensor(
            indices, value, (len(user_list), len(item_list))
        )
    ...
    interaction_matrix = get_interaction_matrix(
        complete_item_list, complete_user_list, train_interaction_data
    )
...
def update_model(model: nn.Module, loss, learning_rate):
    ...
    grads = torch.autograd.grad(
        outputs=loss,
        inputs=filter(lambda x: x.requires_grad, model.parameters()),
        allow_unused=True,
    )
    global time7
    time7 = time.time()

    for name_and_param, grad in zip(
        filter(lambda x: x[1].requires_grad, model.named_parameters()),
        filter(lambda x: x is not None, grads),
    ):
...
def update_model_light(model: MFModel_light, loss, learning_rate):
    ...
    grads = torch.autograd.grad(outputs=loss, inputs=paras)
    global time7
    time7 = time.time()
    # print(grads[0])
...
def calculate_loss_light(
    ...
    interaction_matrix_shard = (
        interaction_matrix.index_select(
            dim=0 if mode == "user" else 1,
            index=torch.tensor(input_user_ids if mode == "user" else input_items_ids),
        )
        .to_dense()
        .to(device)
    )
...
if __name__ == "__main__":
    ...
    interaction_matrix, complete_item_list, complete_user_list = (
        ensure_consistency_of_items_across_datasets()
    )
    ...
    for epoch in range(total_epoch_number):
        ...
        while len(user_id_pool) > 0:
            ...
            time2 = time.time()
            loss = calculate_loss_light(
                model, input_user_ids, input_items_ids, "user", interaction_matrix
            )
            time6 = time.time()
            update_model_light(model, loss, lr / M)

            if len(user_id_pool) == 0:
                log_info = "Epoch: {}-{} \t{}/{} \tLoss: {:.6f}".format(
                    epoch, "user", N - len(user_id_pool), N, loss.item()
                )
                print(log_info)
        ...
        while len(item_id_pool) > 0:
            ...
            loss = calculate_loss_light(
                model, input_user_ids, input_items_ids, "item", interaction_matrix
            )
            update_model_light(model, loss, lr / N)
            if len(item_id_pool) == 0:
                log_info = "Epoch: {}-{} \t{}/{} \tLoss: {:.6f}".format(
                    epoch, "item", M - len(item_id_pool), M, loss.item()
                )
                print(log_info)
